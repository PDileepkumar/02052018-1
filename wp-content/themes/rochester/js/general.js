...
jQuery(window).load(function() {
	...
	rochester_initBlogGrid();
	rochester_initBlogInfiniteScroll();
...
(function($) {
	...
	var ROCHESTER = window.ROCHESTER || {};
	window.ROCHESTER = ROCHESTER;
	
	ROCHESTER.CommonUtils = function() {
		$('a').hover(function() {
	...
	ROCHESTER.IsotopeLayout = function() {
		if( $('.tpath-isotope-layout').length > 0 ) {
			...
			var colWidth = function(index) {
				...
				onLayout = function(isotopeObj, startIndex) {
					...
					setTimeout(function() {
						window.dispatchEvent(ROCHESTER.boxEvent);
	...
	ROCHESTER.VcAnimations = function() {
	...
	ROCHESTER.VcProgressBar = function() {
	...
	ROCHESTER.VcCustomCss = function() {
		window.VcCustomCssInit = function() {
			...
			if( css !== null ) {
				$('head').append( '<style type="text/css" data-type="rochester_vc_custom_styles">' + css + '</style>');
			}
		}
	...
	ROCHESTER.VcEventSlider = function() {
	...
	ROCHESTER.WooAddToCart = function() {
	...
	ROCHESTER.init = function() {
		ROCHESTER.CommonUtils();
		ROCHESTER.IsotopeLayout();
		ROCHESTER.VcAnimations();		
		ROCHESTER.VcProgressBar();
		ROCHESTER.VcCustomCss();
		ROCHESTER.VcEventSlider();
		ROCHESTER.WooAddToCart();
	}
	ROCHESTER.init();
...
jQuery(document).ready(function($){
	...
	(function($) {
		...
		$('.tpath-header-logo-bar .btn-search-trigger').on('click', function() {
			$(this).parents('.tpath-header-logo-bar').find('#header-toggle-search-1').fadeToggle("fast");
			$(this).parents('.tpath-header-logo-bar').find('#header-toggle-search input.form-control').focus();
			$(this).parents('#header-main').addClass("header-search-visible");
			$(this).parents('#header-logo-bar').addClass("header-search-visible");
		});
		...
		$('.tpath-contact-form').each(function(){
			...
			$('#' + contact_form_id).bootstrapValidator({
				...
				$.ajax({
					...
					type: "POST",
					dataType: 'json',
					data: data + '&action=rochester_sendmail',
		...
		$('.tpath-vc-mailchimp-form').each(function(){
			$(this).bootstrapValidator({
				...
				$.ajax({
					...
					type: "POST",
					dataType: 'json',
					data: data + '&action=rochester_mailchimp_subscribe',
					success: function (msg) {
						if( msg.data !== '' ) {
...
function rochester_initBlogGrid() {
...
function rochester_initBlogInfiniteScroll() {
	...
	jQuery('.tpath-posts-container.scroll-infinite').each(function() {
		...
		jQuery( $infinite_container ).infinitescroll({
			...
			loading	  	 : {
							finishedMsg: '<span class="all-loaded">'+ rochester_js_vars.rochester_scroll_text +'</span>',
							msg: null,
							img: rochester_js_vars.rochester_template_uri + "/images/ajax-loader.gif",
							msgText: "",
			},
...
function rochester_ajax_complete() {	
	rochester_CartRemoveItem();
}
